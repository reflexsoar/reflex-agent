...
class ThresholdRule(BaseRule):
    ...
    def execute(self):
        data =  self.conn.search(index=self.detection_input['index'], body=self.query)
        ...
        if len(key_fields) == 0:
            ...
            if self.test_threshold(hits, operator, threshold):
                print(f"[!] Rule \"{rule_name}\" has matched - {hits} {operator} {threshold}")

        if len(key_fields) == 1:
            threshold_values = self.get_bucket_count(key_fields, key_fields[0], data['aggregations'], mode)
            if mode == 'cardinality':
                if self.test_threshold(len(threshold_values), operator, threshold):
                    print(f"[!] Rule \"{rule_name}\" has matched - {len(threshold_values)} {operator} {threshold} - {','.join([str(x['key']) for x in threshold_values])}")
            else:
                for threshold_value in threshold_values:
                    if self.test_threshold(threshold_value['doc_count'], operator, threshold):
                        print(f"[!] {threshold_value['key']} has matched rule \"{rule_name}\" - {threshold_value['doc_count']} {operator} {threshold}")

        elif len(key_fields) >= 2:
            for bucket in data['aggregations'][key_fields[0]]['buckets']:
                threshold_values = self.get_bucket_count(key_fields[1:], threshold_field, bucket, mode)
                if mode == 'cardinality':
                    if self.test_threshold(len(threshold_values), operator, threshold):
                        print(f"[!] {bucket['key']} has matched rule \"{rule_name}\" - {len(threshold_values)} {operator} {threshold} - {','.join([str(x['key']) for x in threshold_values])}")
                else:
                    for threshold_value in threshold_values:
                        if self.test_threshold(threshold_value['doc_count'], operator, threshold):
                            print(f"[!] {bucket['key']} has matched rule \"{rule_name}\" - {threshold_value['doc_count']} {operator} {threshold} - {','.join([str(x['key']) for x in threshold_values])}")
